...
# @pytest.fixture
# def api_client():
#     """Initialize API client with TASKLY_DEV config"""
...
#     # For our API, successful response is a dict
#     assert isinstance(response, dict), "❌ FAILED: Response is not valid"
#     print(f"✅ Test 1 PASSED: HTTP 200 - Got valid response")


# # ============ TEST 2: JSON Format Check ============
# def test_2_response_json_format(api_client):
#     """Verify API response is proper JSON format (dict)"""
#     response = api_client.call_api("What is RIM?")
    
#     # Check if response is valid JSON (dict)
#     assert isinstance(response, dict), "❌ FAILED: Response is not JSON dict"
    
#     # Check for expected fields
#     assert "answer" in response or "message" in response, "❌ FAILED: No answer/message in response"
    
#     print(f"✅ Test 2 PASSED: Valid JSON format with {len(response)} fields")


# # ============ TEST 3: CSV Questions - Read & Output ============
# def test_3_csv_questions_proper_output(api_client, csv_questions):
#     """Read questions from CSV and verify API returns proper output"""
...
#         if isinstance(response, dict) and ("answer" in response or "message" in response):
#             passed += 1
#         else:
#             failed += 1
    
#     assert failed == 0, f"❌ FAILED: {failed} questions had invalid responses"
#     print(f"✅ Test 3 PASSED: {passed}/{passed + failed} CSV questions got proper output")


# # ============ TEST 4: Response Time ============
# def test_4_response_time_check(api_client):
#     """Verify API response time is within acceptable limit"""
#     start = time.time()
#     response = api_client.call_api("What is TEH?")
#     duration = time.time() - start
    
#     # Check response is valid
#     assert isinstance(response, dict), "❌ FAILED: Invalid response"
    
#     # Check time limit (30 seconds)
#     assert duration < 30, f"❌ FAILED: Response took {duration:.2f}s (limit: 30s)"
    
#     print(f"✅ Test 4 PASSED: Response time {duration:.2f}s (limit: 30s)")
